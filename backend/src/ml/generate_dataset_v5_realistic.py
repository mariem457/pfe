"""
generate_dataset_v4_fixed.py
Dataset Smart Waste Management – Paris 15e – VERSION V5 REALISTIC
"""
...
OUT_FILE       = "final_dataset_paris15_v5_realistic.csv"
...
# Seuils plus réalistes: on ne collecte pas à 55%, sauf urgence.
...
# Le weekend dépend aussi de l’activité de la zone.
...
# Pics d’activité humains: matin, midi, soir, nuit.
PEAK_HOURS_FACTOR = {
    "morning": 1.15,
    "lunch":   1.20,
    "evening": 1.30,
...
# Pour éviter des changements brutaux du fill_rate d’une heure à l’autre.
TEMPORAL_MEMORY_ALPHA = 0.75

# Pour le dispatching, au-delà de 48h ce n’est pas opérationnellement utile.
TIME_TO_FULL_TARGET_LEVEL = 90
TIME_TO_FULL_MAX_HOURS = 48
...
def should_collect(type_bin, dow, hour, fill_level):
    ...
    # Collecte planifiée uniquement si le bac est vraiment proche du seuil utile.
    if (dow in sched["days"]) and (hour == sched["hour"]) and fill_level >= threshold:
        return True

    # Collecte urgente si presque plein.
    if fill_level >= 92:
        return True
...
for _, bin_row in bins_df.iterrows():
    ...
    # Mémoire temporelle: un bac garde une dynamique proche d’une heure à l’autre.
    ...
    for _, w in weather_df.iterrows():
        ...
        # Mémoire temporelle / smoothing:
        # 75% de la dynamique précédente + 25% du contexte courant.
        fill_rate_true = (
            TEMPORAL_MEMORY_ALPHA * previous_fill_rate
            + (1.0 - TEMPORAL_MEMORY_ALPHA) * raw_rate
        )
        fill_rate_true = max(0.01, fill_rate_true)
        ...
        all_records.append({
            ...
            "is_morning_peak": int(7 <= hour_ <= 9),
            "is_lunch_peak": int(12 <= hour_ <= 14),
            "is_evening_peak": int(18 <= hour_ <= 21),
            "zone_weekend_factor": round(zone_weekend_factor, 3),
...
df["time_to_full"] = np.where(
    ...
    (TIME_TO_FULL_TARGET_LEVEL - df["fill_level"]) / df["fill_rate"],
    np.nan
)

# Si le bac est déjà >= 90%, time_to_full = 0.
# Si le délai dépasse 48h, on le clippe: pas utile pour la planification opérationnelle.
df["time_to_full"] = df["time_to_full"].clip(lower=0, upper=TIME_TO_FULL_MAX_HOURS)

# Features dérivées utiles pour le LSTM, calculées après assemblage.
...
# Après une collecte ou une perte signal, les deltas peuvent être NaN: on met 0.
df[["fill_delta_1h", "fill_delta_3h", "fill_delta_6h"]] = (
    df[["fill_delta_1h", "fill_delta_3h", "fill_delta_6h"]].fillna(0)
)
...
print("\n" + "=" * 60)
print("VALIDATION DATASET V5 REALISTIC")
print("=" * 60)
