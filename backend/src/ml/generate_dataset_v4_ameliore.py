...
Dataset Smart Waste Management – Paris 15e – VERSION FIXED
"""
...
OUT_FILE       = "final_dataset_paris15_v4_fixed.csv"
...
def should_collect(type_bin, dow, hour, fill_level):
    ...
    if (dow in sched["days"]) and (hour == sched["hour"]) and fill_level >= 55:
        return True
...
for _, bin_row in bins_df.iterrows():
    ...
    for _, w in weather_df.iterrows():
        ...
        if np.random.random() < event_probability and 7 <= hour_ <= 22:
            event_factor = float(np.random.uniform(1.10, 1.80) * bin_event_sensitivity)
        ...
        fill_rate_true = max(0.01, effective_rate * noise_mult)
