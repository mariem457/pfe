...
public class BinService {
    ...
    public BinResponse create(BinRequest req) {
        ...
        Bin b = new Bin();
        apply(b, req, true);
        Bin saved = binRepository.save(b);

        // QR code généré on-demand via GET /api/bins/{id}/qrcode
        // Pas de stockage en base nécessaire — toujours généré depuis binCode
    ...
    public void processTelemetry(BinTelemetryDTO dto) {
        ...
        Optional<BinTelemetry> previousOpt =
                binTelemetryRepository.findTopByBinOrderByTimestampDesc(bin);
        ...
        if (!"MQTT_REAL".equals(src) && !"MQTT_SIM".equals(src)) src = "MQTT_SIM";
        ...
        double hour        = saved.getTimestamp().atZone(ZoneId.systemDefault()).getHour();
        ...
        if (previousTelemetry != null) {
            ...
            if (hoursDiff > 0) fillRatePerHour = fillDelta / hoursDiff;
        }

        double fillLevelLag1 = previousTelemetry != null
                ? previousTelemetry.getFillLevel() : saved.getFillLevel();
        double fillLevelLag2 = secondPreviousTelemetry != null
                ? secondPreviousTelemetry.getFillLevel() : fillLevelLag1;
        double fillRateLag1  = 0.0;
        if (previousTelemetry != null && secondPreviousTelemetry != null) {
            fillRateLag1 = Math.max(0.0,
    ...
    private void apply(Bin b, BinRequest req, boolean isCreate) {
        if (isCreate || req.binCode != null) b.setBinCode(req.binCode);
        ...
        if (isCreate || req.lat       != null) b.setLat(req.lat);
        if (isCreate || req.lng       != null) b.setLng(req.lng);
        if (isCreate || req.accessLat != null) b.setAccessLat(req.accessLat);
        if (isCreate || req.accessLng != null) b.setAccessLng(req.accessLng);
        ...
        if (b.getAccessLat() == null) b.setAccessLat(b.getLat());
        if (b.getAccessLng() == null) b.setAccessLng(b.getLng());
    ...
    private String generateNextBinCode() {
        int max = 0;
        for (Bin b : binRepository.findAll()) {
    ...
    private BinResponse toResponse(Bin b) {
        ...
        Optional<BinTelemetry> latestOpt =
                binTelemetryRepository.findTopByBinOrderByTimestampDesc(b);

        if (latestOpt.isPresent()) {
            BinTelemetry latest = latestOpt.get();
