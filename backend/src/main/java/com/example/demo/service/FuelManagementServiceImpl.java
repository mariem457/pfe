...
public class FuelManagementServiceImpl implements FuelManagementService {
    ...
    @Override
    public boolean canSafelyCompleteRoute(Truck truck, double routeDistanceKm) {
        if (truck == null || routeDistanceKm <= 0) {
            return false;
        }

        if (isFuelCritical(truck)) {
            return false;
        }

        return calculateEstimatedAutonomyKm(truck) >= routeDistanceKm;
    }
