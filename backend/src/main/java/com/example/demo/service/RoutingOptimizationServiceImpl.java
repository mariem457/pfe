...
public class RoutingOptimizationServiceImpl implements RoutingOptimizationService {
    ...
    public RoutingOptimizationServiceImpl(TruckRepository truckRepository,
                                          ...
                                          FleetEligibilityService fleetEligibilityService,
        ...
        this.fleetEligibilityService = fleetEligibilityService;
    ...
    public RoutingResponseDto prepareInitialRouting() {
        ...
        List<Truck> eligibleTrucks = activeTrucks.stream()
                .filter(truck -> fleetEligibilityService.isTruckEligible(truck, 100.0, 25.0))
                .toList();

        System.out.println("Eligible trucks count: " + eligibleTrucks.size());

        for (Truck truck : eligibleTrucks) {
            System.out.println(
                    "Eligible truck -> id=" + truck.getId()
                            + ", code=" + truck.getTruckCode()
                            + ", status=" + truck.getStatus()
            );
        }

        RoutingRequestDto routingRequest = routingPayloadBuilderService.buildRoutingRequest(eligibleTrucks);
    ...
    public List<MissionResponse> planAndSaveMissions() {
        ...
        if (eligibleTrucks.isEmpty()) {
            throw new BadRequestException("No eligible trucks available for routing");
        }
        ...
        if (routingResponse == null || routingResponse.getMissions() == null || routingResponse.getMissions().isEmpty()) {
            throw new BadRequestException("Routing engine returned no missions");
        }

        List<MissionResponse> savedMissions = new ArrayList<>();

        for (RoutingMissionDto routingMissionDto : routingResponse.getMissions()) {
            Mission savedMission = saveMissionFromRouting(routingMissionDto, eligibleTrucks, routingRequest);
    ...
    private Mission saveMissionFromRouting(RoutingMissionDto routingMissionDto,
        ...
        Truck truck = eligibleTrucks.stream()
    ...
    private void saveRoutePlanAndStops(Mission mission,
        ...
        if (routingMissionDto.getStops() != null) {
            for (RoutingStopDto stopDto : routingMissionDto.getStops()) {
                ...
                // IMPORTANT: use access point for routing if available
