...
public class MissionServiceImpl implements MissionService {
    ...
    private MissionFuelStatusResponse buildMissionFuelStatus(Mission mission, RoutePlan latestPlan) {
        ...
        double routeDistanceKm = latestPlan.getTotalDistanceKm() != null
        ...
        dto.setSafeAutonomyKm(round(autonomyKm));
        ...
        dto.setTriggerDistanceKm(
                distanceToStationKm == Double.MAX_VALUE
                        ? null
        ...
        if (fuelManagementService.isFuelCritical(truck) || !canReachStationSafely) {
            dto.setFuelStatus("CRITICAL");
            dto.setMessage("Carburant critique. Le camion doit rejoindre une station avant de continuer.");
            dto.setDecisionReason(
                    "Le carburant est critique ou l'autonomie sécurisée ("
                            + round(autonomyKm)
                            + " km) est inférieure à la distance nécessaire pour atteindre la station recommandée avec marge de sécurité ("
                            + round(distanceToStationKm + MIN_DISTANCE_BUFFER_TO_STATION_KM)
                            + " km)."
            );
            return dto;
        ...
        if (refuelStopAlreadyInserted) {
            dto.setFuelStatus("NORMAL");
            dto.setMessage("Une station-service est déjà planifiée dans l'itinéraire.");
            dto.setDecisionReason("Le système a déjà sécurisé la mission par l'insertion d'un arrêt carburant.");
            return dto;
        }

        boolean canCompleteRouteSafely =
                fuelManagementService.canSafelyCompleteRoute(truck, routeDistanceKm);

        if (routeDistanceKm > 0 && !canCompleteRouteSafely) {
            int suggestedOrder = estimateSuggestedInsertionOrder(latestPlan, autonomyKm);

            dto.setFuelStatus("REFUEL_REQUIRED");
            dto.setMessage("Autonomie insuffisante pour terminer la mission. Un arrêt carburant est recommandé.");
            dto.setDecisionReason(
                    "L'autonomie sécurisée restante ("
                            + round(autonomyKm)
                            + " km) est inférieure à la distance totale estimée de la mission ("
                            + round(routeDistanceKm)
                            + " km)."
            );
            dto.setSuggestedInsertionOrder(suggestedOrder);
            dto.setRouteInsertionReason(
                    "Insertion recommandée avant l'arrêt numéro "
                            + suggestedOrder
                            + " afin de sécuriser la suite de la tournée."
            );
            return dto;
        }
        ...
        dto.setDecisionReason(
                "L'autonomie sécurisée ("
                        + round(autonomyKm)
                        + " km) est suffisante pour couvrir la distance totale estimée de la mission ("
                        + round(routeDistanceKm)
                        + " km)."
        );
